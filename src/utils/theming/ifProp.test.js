...
describe('ifProp', () => {
   ...
   test('function array argument', () => {
      expect(ifProp([props => props.foo], 'yes', 'no')({ bar: false, foo: true })).toBe('yes');
   });

   test('object array argument', () => {
      expect(ifProp([{ foo: true }], 'yes', 'no')({ bar: false, foo: true })).toBe('yes');
   });
   ...
   test('function values', () => {
      expect(ifProp('foo', props => props.foo, props => props.bar)({ bar: 'bar' })).toBe('bar');
      expect(ifProp('foo', props => props.foo, props => props.bar)({ foo: 'foo' })).toBe('foo');
   });
});
